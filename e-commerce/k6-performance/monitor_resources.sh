...
export PGPASSWORD="${PGPASSWORD:-220711833}"
...
TARGET_PID=$(ss -tlnp | grep ":$TARGET_PORT " | grep -oP "pid=\K[0-9]+" | head -1 || echo "")
...
OTHER_PID=$(ss -tlnp | grep ":$OTHER_PORT " | grep -oP "pid=\K[0-9]+" | head -1 || echo "")
if [ -n "$OTHER_PID" ]; then
  echo ""
