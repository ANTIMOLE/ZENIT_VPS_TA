...
describe("productService.getAll()", () => {
  ...
  it("✅ sort field camelCase diteruskan ke Prisma", async () => {
    mockCount.mockResolvedValue(1);
    mockFindMany.mockResolvedValue([fakeProduct]);

    await productService.getAll({ sortBy: "soldCount", sortOrder: "desc", page: 6 });
  ...
  it("✅ sort field createdAt diteruskan ke Prisma", async () => {
    mockCount.mockResolvedValue(1);
    mockFindMany.mockResolvedValue([fakeProduct]);

  await productService.getAll({ sortBy: "createdAt", sortOrder: "asc", page: 7 });
  const orderBy = mockFindMany.mock.calls[0]?.[0]?.orderBy;
