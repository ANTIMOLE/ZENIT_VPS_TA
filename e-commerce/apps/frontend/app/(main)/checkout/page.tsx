        ...
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <Section step={1} icon={<MapPin className="w-4 h-4" />} title="Alamat Pengiriman">
              {isLoadingAddr ? (
                ...
                <div className="text-center py-4">
                  <p className="text-sm text-zinc-500 mb-3">Belum ada alamat tersimpan.</p>
                ...
                <div className="space-y-2">
                  {addresses.map(addr => (
                    <button
                      ...
                      className={cn(
                        "w-full text-left p-3 rounded-xl border-2 transition-all duration-150 text-sm",
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <p className="font-medium text-zinc-800">{addr.recipientName}</p>
                          <p className="text-zinc-500 text-xs mt-0.5">{addr.phone}</p>
                  ...
                  <Button variant="ghost" size="sm" className="w-full gap-1.5 text-zinc-500 mt-1" onClick={() => setShowAddAddr(true)}>
                ...
                {SHIPPING_OPTIONS.map(opt => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-zinc-800">{opt.name}</p>
              ...
              <div className="space-y-2">
                {PAYMENT_METHODS.map(method => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-zinc-800">{method.name}</p>
          ...
          <div className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-white rounded-2xl border border-zinc-200 p-5 sticky top-20">
              <h2 className="font-semibold text-base text-zinc-900 mb-4">Ringkasan Order</h2>
                  ...
                  {cart?.items?.map(item => (
                    <div key={item.id} className="flex items-center gap-2 text-xs">
                      <div className="relative w-8 h-8 rounded-lg overflow-hidden bg-zinc-100 flex-shrink-0">
                        <ProductThumb images={item.product?.images} alt={item.product.name} sizes="32px" />
                      </div>
                      <p className="flex-1 text-zinc-600 line-clamp-1">{item.product.name}</p>
                      <p className="font-medium text-zinc-800 flex-shrink-0">×{item.quantity}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-zinc-800">{formatPrice(displaySubtotal)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-500">PPN 11%</span>
                  <span className="text-zinc-800">{formatPrice(displayTax)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-500">Ongkir</span>
                  {/* [FIX] Tampilkan "..." saat summary sedang loading — hindari angka lama */}
                  <span className="text-zinc-800">{isLoadingSummary ? "..." : formatPrice(displayShipping)}</span>
                </div>
              </div>
              ...
              {!selectedAddressId && (
                <p className="text-xs text-center text-zinc-400 mt-2">Pilih alamat pengiriman dulu</p>
              )}
            </div>
          </div>
...
function Section({ step, icon, title, children }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-zinc-200">
      <div className="flex items-center gap-2.5 px-5 py-4 border-b border-zinc-200">
