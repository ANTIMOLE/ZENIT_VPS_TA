    ...
    <div className="min-h-screen bg-[#fafafa]">
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Kelola informasi akun kamu</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200">
          <div className="flex items-center gap-4 p-6 border-b border-zinc-200">
            ...
            <div>
              <p className="font-semibold text-lg text-zinc-900">{displayUser?.name ?? "—"}</p>
          ...
          <div className="p-6 space-y-4">
            {editMode ? (
              <div className="space-y-4">
                <div>
                  <Label htmlFor="name" className="text-sm font-medium text-zinc-600">Nama Lengkap</Label>
                  <Input id="name" value={form.name} className="mt-1"
                    onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                    placeholder="Nama lengkap kamu" />
                </div>
                <div>
                  <Label htmlFor="phone" className="text-sm font-medium text-zinc-600">Nomor Telepon</Label>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-zinc-500 w-24 flex-shrink-0">Nama</span>
                  <span className="font-medium text-zinc-800">{displayUser?.name ?? "—"}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <Mail className="w-4 h-4 text-zinc-400 flex-shrink-0" />
                  <span className="text-zinc-500 w-24 flex-shrink-0">Email</span>
                  <span className="font-medium text-zinc-800">{displayUser?.email ?? "—"}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <Phone className="w-4 h-4 text-zinc-400 flex-shrink-0" />
                  <span className="text-zinc-500 w-24 flex-shrink-0">Telepon</span>
                  <span className="font-medium text-zinc-800">{displayUser?.phone ?? "—"}</span>
                </div>
                <div className="pt-2">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200">
          ...
          {loadingAddr && (
            <div className="p-4 space-y-3">
              {[1, 2].map(i => (
                <div key={i} className="rounded-xl border border-zinc-200 p-4 space-y-2">
                  <Skeleton className="h-4 w-32" />
          ...
          {!loadingAddr && addresses.length === 0 && (
            <div className="flex flex-col items-center justify-center py-10 text-center px-6">
              ...
              <p className="text-sm font-medium text-zinc-600">Belum ada alamat</p>
              <p className="text-xs text-zinc-400 mt-1">Tambah alamat pengiriman untuk checkout lebih cepat</p>
          ...
          {!loadingAddr && addresses.length > 0 && (
            <div className="divide-y divide-zinc-100">
              {addresses.map(addr => (
                <div key={addr.id} className="p-4 hover:bg-zinc-50/60 transition-colors">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {!addr.isDefault && (
                        <Button
                          ...
                          className="h-7 px-2 text-xs text-zinc-500 hover:text-primary"
                          disabled={isMutating}
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-zinc-200">
            <p className="font-semibold text-sm text-zinc-900">Aktivitas Saya</p>
          </div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-sm font-medium text-zinc-800">Pesanan Saya</p>
                <p className="text-xs text-zinc-500">Lihat semua riwayat pesanan</p>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-zinc-500">Ubah kata sandi akun</p>
              </div>
            </div>
            <ChevronRight className="w-4 h-4 text-zinc-400" />
          </button>
            ...
            <div className="space-y-3 py-1">
              ...
              {!editingAddr?.isDefault && (
                <label className="flex items-center gap-2.5 cursor-pointer pt-1">
                  ...
                  />
                  <span className="text-sm text-zinc-600">Jadikan alamat utama</span>
                </label>
              )}
            </div>
