/**
 * cart.service.test.ts — Whitebox Unit Test (backend-rest)
 *
 * Letakkan di: backend-rest/src/__tests__/unit/cart.service.test.ts
 *
 * Menguji: getCartByUserId, addItemToCart, updateCartItem,
 *          removeCartItem, clearCart
 *
 * Perilaku kritis yang dikunci:
 *  - checked_out → buat cart BARU (bukan reaktivasi)
 *  - item stock=0 otomatis dihapus
 *  - item quantity > stock di-cap ke stock
 *  - harga diskon dikalkulasi saat addItem
 */
...
describe("getCartByUserId()", () => {
  ...
  it("✅ buat cart BARU jika status checked_out (BUKAN reaktivasi via update)", async () => {
    ...
    // Harus CREATE, bukan UPDATE — supaya checked_out bisa jadi audit trail
    expect(mockCart.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: { userId: "user-1", status: "active" } })
    );
  ...
  it("✅ item dengan stock 0 otomatis dihapus dari cart", async () => {
    ...
    const cartWithOOS = makeCart({ items: [outOfStockItem] });
    const cartAfterDelete = makeCart({ items: [] });
    mockCart.findUnique.mockResolvedValue(cartWithOOS);
    mockCart.update.mockResolvedValue(cartAfterDelete);
  ...
  it("✅ item quantity > stock di-cap ke nilai stock", async () => {
    // stock: 3, quantity: 10 → harus di-cap ke 3
    const overQtyItem = makeItem({ quantity: 10, product: makeProduct({ stock: 3 }) });
    const cartWithOver = makeCart({ items: [overQtyItem] });
    const cartCapped = makeCart({ items: [makeItem({ quantity: 3 })] });
    mockCart.findUnique.mockResolvedValue(cartWithOver);
    mockCart.update
      .mockResolvedValueOnce(cartCapped) // cap quantity
      .mockResolvedValueOnce(cartCapped); // sync harga (jika ada)
...
describe("addItemToCart()", () => {
  ...
  it("✅ update quantity jika produk sudah ada di cart (quantity di-akumulasi)", async () => {
  ...
  it("❌ throw 404 jika produk tidak ditemukan", async () => {
    ...
    await expect(addItemToCart("user-1", "ghost-prod", 1))
  ...
  it("❌ throw 400 jika total quantity (existing + baru) melebihi stok", async () => {
    ...
    // existing 8 + tambah 5 = 13 > stock 10 → harus reject
    await expect(addItemToCart("user-1", "prod-1", 5))
      .rejects.toMatchObject({ status: 400 });
  });

  it("✅ harga diskon dikalkulasi dengan benar saat tambah item", async () => {
    mockCart.findUnique.mockResolvedValue(makeCart({ items: [] }));
    mockProduct.findUnique.mockResolvedValue({
  ...
  it("✅ harga tanpa diskon = harga asli", async () => {
    mockCart.findUnique.mockResolvedValue(makeCart({ items: [] }));
    mockProduct.findUnique.mockResolvedValue({
...
describe("removeCartItem()", () => {
  ...
  it("❌ throw 404 jika item tidak ada di cart user (ownership check)", async () => {
    mockCart.findUnique.mockResolvedValue(makeCart({ items: [] }));
